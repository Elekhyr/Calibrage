...
void Camera::Calibrate(const std::vector<cv::Mat>& calibrationImages, Chessboard chessboard)
{
	...
	//cv::fisheye::calibrate(worldSpaceCorners, imageSpaceCorners, calibrationImages.front().size(), cameraMatrix, distCoefficients, rVectors, tVectors);
...
void Camera::corrige(const cv::Mat& imageTordue, cv::Mat& imageCorrige){
	...
	//cv::fisheye::undistortImage(imageTordue, imageCorrige, cameraMatrix, distCoefficients);
	cv::undistort(imageTordue, imageCorrige, cameraMatrix, distCoefficients);
}
